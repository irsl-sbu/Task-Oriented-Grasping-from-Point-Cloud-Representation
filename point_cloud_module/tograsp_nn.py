'''Python class for:
    ...
    3) Given a constant screw motion represented using a screw axis (unit vector and a point), 
       a pretrained MLP along with the bounding box corresponding to the object's point cloud
...
class tograsp():
    ...
    def __init__(self, cloud_object):
        ...
        self.grasp_metric_threshold = None
    ...
    def generate_contacts_xz_plucker_non_plucker(self):
      ...
      self.sampled_c1 = np.asarray([[x, self.cloud_object.transformed_vertices_object_frame[0,1], z] for z in self.z_axis_increments for x in self.x_axis_increments])
      self.sampled_c2 = np.asarray([[x, self.cloud_object.transformed_vertices_object_frame[2,1], z] for z in self.z_axis_increments for x in self.x_axis_increments])
      self.x_data = np.asarray([np.reshape(np.asarray([[x, self.cloud_object.transformed_vertices_object_frame[0,1], z], [x, self.cloud_object.transformed_vertices_object_frame[2,1], z], self.unit_vector, self.moment, self.point]), [1,15]) for z in self.z_axis_increments for x in self.x_axis_increments])
      self.x_data = np.reshape(self.x_data, [self.x_data.shape[0], self.x_data.shape[1]*self.x_data.shape[2]])
    ...
    def project_points_yz(self):
      ...
      vect_2 = np.subtract(self.cloud_object.transformed_vertices_object_frame[7, :], self.cloud_object.transformed_vertices_object_frame[1, :])

      # Computing the center point on the plane just for visualization: 
      center_point_plane = np.asarray([[self.cloud_object.transformed_vertices_object_frame[1, 0]], self.cloud_object.p_base[1], self.cloud_object.p_base[2]])
