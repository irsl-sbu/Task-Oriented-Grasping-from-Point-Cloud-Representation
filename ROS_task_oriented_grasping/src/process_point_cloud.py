...
class pointCloud(object):
   ...
   def __init__(self): 
      ...
      # The point coordinates with the highest metric value correspond to the ideal grasping region.
      # They are expressed in the object body reference frame. 
      self.highest_metric_values_points = None
      self.highest_metric_values_normals = None
      self.highest_metric_values = None
      self.highest_metric_values_indices = None
   ...
   def getDistance(self, plane_points, center_point, grasp_center):
      ...
      # Translating each of the points by a specified distance: 
      # Computing the D in the equation of the plane Ax + By + Cz + D = 0:
      D = np.dot(center_point, unit_n)

      num = la.norm(unit_n[0]*grasp_center[0] + unit_n[1]*grasp_center[1] + unit_n[2]*grasp_center[2] - D)
      den = np.sqrt(unit_n[0]**2 + unit_n[1]**2 + unit_n[2]**2)
      distance = np.divide(num, den)
      return distance

   '''Function to sample end effector poses based on the predicted metric values.'''
   # This function needs to be updated: 
   def getEndEffectorPoses(self):
      # Saving the sampled end effector poses:
      self.sampled_end_effector_poses = []
      ...
      for i in range(len(self.highest_metric_values_points)):
         max_element = np.amax(np.absolute(self.highest_metric_values_normals[i]))
         max_idx = np.where(abs(self.highest_metric_values_normals[i]) == max_element)
         if max_idx[0] == 0:
            ...
            if self.x_dim < self.gripper_width_tolerance:
               ...
               # Frame C1:
               self.z_C1 = -1*self.highest_metric_values_normals[i]
               ...
               # Frame C2:
               self.z_C2 = self.highest_metric_values_normals[i]
               ...
               self.position_C1 = self.highest_metric_values_points[i, :]
               ...
               self.distance_1 = self.getDistance(self.plane_points_1, self.center_point_plane_1, self.grasp_center)
               self.distance_2 = self.getDistance(self.plane_points_2, self.center_point_plane_2, self.grasp_center)
               self.distance_3 = self.getDistance(self.plane_points_3, self.center_point_plane_3, self.grasp_center)
               ...
               if self.distance_1 < self.gripper_height_tolerance:
                  ...
                  self.p_EE = np.add(self.grasp_center, np.dot(-0.1034, self.z_EE))   
                  ...
                  self.sampled_end_effector_poses.append(self.gripper_pose)
            ...
            if self.y_dim < self.gripper_width_tolerance:
               ...
               if self.distance_2 < self.gripper_height_tolerance: 
                  ...
                  self.y_EE = np.asarray([0,-1,0])
                  self.x_EE = np.asarray([0,0,1])
                  self.z_EE = np.cross(self.x_EE, self.y_EE)
                  self.R_EE = np.zeros([3,3])
                  self.R_EE[:, 0] = self.x_EE
                  self.R_EE[:, 1] = self.y_EE
                  self.R_EE[:, 2] = self.z_EE
      ...
      self.sampled_end_effector_poses_base = []
 
      for pose in self.sampled_end_effector_poses:
         R_pose = pose[0:3, 0:3]
         ...
         self.sampled_end_effector_poses_base.append(self.gripper_pose_base) 
