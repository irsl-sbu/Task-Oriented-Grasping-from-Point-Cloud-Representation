...
if __name__ == "__main__":
    ...
    # Specifying the screw parameters (Screw Driver): 
    '''cloud_object.screw_axis = np.asarray([0, -1, 0])
    cloud_object.point = np.asarray([0, cloud_object.transformed_vertices_object_frame[1,1], 0])
    cloud_object.moment = np.cross(cloud_object.point, cloud_object.screw_axis)'''
    ...
    # Specifying the screw parameters (Screw Driver): 
    cloud_object.screw_axis = np.asarray([-1, 0, 0])
    cloud_object.point = np.asarray([cloud_object.transformed_vertices_object_frame[0,0], 0, 0])
    cloud_object.moment = np.cross(cloud_object.point, cloud_object.screw_axis)
    ...
    np.savetxt("screwdriver_pose3_EE1_object_frame.csv", cloud_object.transformed_points_object_frame, delimiter= ",")
    o3d.io.write_point_cloud("screwdriver_pose3_EE1_segmented.ply", cloud_object.cloud_object_frame)
    ...
    # Specifying the screwing motion: 
    R_final_1 = np.asarray([[1, 0, 0],
                           [0, -1, 0],
                          [0, 0, -1]])
    p_final_1 = np.reshape(np.asarray([0.5, 0.1, 0.6]), [3])
    g_final_1 = np.eye(4)
    g_final_1[0:3, 0:3] = R_final_1
    g_final_1[0:3, 3] = p_final_1

    # Computing the desired rotation about the screw axis:
    axis = np.asarray([1,0,0])
    theta = math.radians(40)
    pitch = 0
    point = p_final_1 - np.asarray([0, 0, 0.1234])
    g = getTransformationForScrew(axis, pitch, theta, point)
    g_final_2 = np.matmul(g, g_final_1)
    ...
    ax13.scatter(x_transformed_points, y_transformed_points, z_transformed_points, s = 0.2)
    '''ax12.scatter(cloud_object.point[0], cloud_object.point[1], cloud_object.point[2], marker = '*', s = 100, color = 'r')
    ax12.quiver(cloud_object.point[0], cloud_object.point[1], cloud_object.point[2], 0.25*cloud_object.screw_axis[0], 0.25*cloud_object.screw_axis[1], 0.25*cloud_object.screw_axis[2], color = "r", arrow_length_ratio = 0.25)'''
    # sc11 = ax11.scatter(x_highest_metric_values_points, y_highest_metric_values_points, z_highest_metric_values_points)
    ...
    cloud_object.scale_value = 0.45
    cloud_object.length_value = 0.25
    ax13 = cloud_object.plotReferenceFrames(ax13)

    # Grasp centers:
    for i in range(len(cloud_object.grasp_centers)):
        g = cloud_object.grasp_centers[i]
        ax13.scatter(g[0], g[1], g[2], marker = '*', s = 20, color = 'r')

    ax13.set_xlabel('X')
    ax13.set_ylabel('Y')
    ax13.set_zlabel('Z')
    ax13.set_xlim(-0.4, 0.4)
    ax13.set_ylim(-0.4, 0.4)
    ax13.set_zlim(-0.4, 0.4)
    plt.show()
    ...
    # Converting the g_final_1 into quaternion:
    rot_quat = Rot.from_matrix(g_final_1[0:3, 0:3])
    rot_quat = rot_quat.as_quat()
    pose_final_1 = Pose()
    pose_final_1.position.x = g_final_1[0,3]
    pose_final_1.position.y = g_final_1[1,3]
    pose_final_1.position.z = g_final_1[2,3]

    pose_final_1.orientation.x = rot_quat[0]
    pose_final_1.orientation.y = rot_quat[1]
    pose_final_1.orientation.z = rot_quat[2]
    pose_final_1.orientation.w = rot_quat[3]

    # Converting the g_final_2 into quaternion:
    rot_quat = Rot.from_matrix(g_final_2[0:3, 0:3])
    rot_quat = rot_quat.as_quat()
    pose_final_2 = Pose()
    pose_final_2.position.x = g_final_2[0,3]
    pose_final_2.position.y = g_final_2[1,3]
    pose_final_2.position.z = g_final_2[2,3]

    pose_final_2.orientation.x = rot_quat[0]
    pose_final_2.orientation.y = rot_quat[1]
    pose_final_2.orientation.z = rot_quat[2]
    pose_final_2.orientation.w = rot_quat[3]

    ee_trajectory = [cloud_object.computed_end_effector_quaternion_inter_base[num_pose], cloud_object.computed_end_effector_quaternion_base[num_pose], pose_final_1, pose_final_2]
    
    print("End Effector Pose: ", cloud_object.computed_end_effector_poses_base[num_pose])
    print("Quaternion end-effector pose: ", cloud_object.computed_end_effector_quaternion_base[num_pose])
    ...
    np.savetxt('screwdriver_pose3_EE2_pre_grasp_poses_object_frame_alg31.csv', pre_grasp_poses_object_frame, delimiter=",")
    np.savetxt('screwdriver_pose3_EE2_grasp_poses_object_frame_alg31.csv', grasp_poses_object_frame, delimiter=",")

    np.savetxt('screwdriver_pose3_EE2_pre_grasp_poses_base_frame_alg31.csv', pre_grasp_poses_base_frame, delimiter=",")
    np.savetxt('screwdriver_pose3_EE2_grasp_poses_base_frame_alg31.csv', grasp_poses_base_frame, delimiter=",")

    # Saving the screw axis and point as expressed in the object reference frame:
    np.savetxt('screwdriver_pose3_EE2_screw_axis_object_frame_alg31.csv', cloud_object.screw_axis, delimiter = ',')
    np.savetxt('screwdriver_pose3_EE2_point_object_frame_alg31.csv', cloud_object.point, delimiter = ',')

    # Saving the pose of the object with respect to the base frame:
    np.savetxt('R_object_alg31.csv', cloud_object.R_bounding_box, delimiter = ',')
    np.savetxt('p_object_alg31.csv', cloud_object.p_bounding_box, delimiter = ',')
